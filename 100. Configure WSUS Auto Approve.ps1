...
Invoke-Command -Session $Session -ScriptBlock {
    ...
    $CriticalUpdates = $WSUSServer.GetUpdateClassifications() | Where-Object -Property Title -eq 'Critical Updates'
    $UpdateClassifications.Add($CriticalUpdates) | Out-Null
    $Defs = $WSUSServer.GetUpdateClassifications() | Where-Object -Property Title -eq 'Definition Updates'
...
Invoke-Command -Session $Session -ScriptBlock {
    ...
    $TargetGroups.Add(($WSUSServer.GetComputerTargetGroups() | Where-Object -Property Name -eq 'Domain Servers'))
    $ApprovalRule.SetComputerTargetGroups($TargetGroups) | Out-Null
}
...
Invoke-Command -Session $Session -ScriptBlock {
    $WSUSServer.GetInstallApprovalRules() | Format-Table -Property Name, Enabled, Action
}