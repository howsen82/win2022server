...
Get-ADUser -Filter * -Property *| Format-Table -Property Name, Displayname, SamAccountName

# 8. Removing via a Get | Remove pattern
Get-ADUser -Identity 'CN=TBR1,OU=IT,DC=Reskit,DC=Org' | Remove-ADUser -Confirm:$false
...
Get-ADUser -Identity ThomasL -Properties * | Format-Table -Property DisplayName,Name,Office,OfficePhone,EmailAddress
...
Get-ADGroupMember -Identity 'IT Team' | Format-Table SamAccountName, DistinguishedName
