...
Get-ADComputer -Filter * | Format-Table -Property Name, DistinguishedName

# 2. Getting computers in the UK Domain
Get-ADComputer -Filter * -Server UKDC1.UK.Reskit.Org | Format-Table -Property Name, DistinguishedName
...
# 6. Testing to ensure SRV1 is online
Test-NetConnection -ComputerName SRV1 -Port 5985
Test-NetConnection -ComputerName SRV1 -port 5985
...
Get-ADComputer -Filter * -Properties DNSHostName | Format-Table name, DNSHostName, Enabled