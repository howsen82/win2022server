...
$IPType    = 'IPv4'
$Adapter   = Get-NetAdapter -Name Ethernet | Where-Object Status -eq 'Up'
$Interface = $Adapter | Select-Object -First 1 | Get-NetIPInterface -AddressFamily $IPType
$Index     = $Interface.IfIndex
Get-NetIPAddress -InterfaceIndex $Index -AddressFamily $IPType | Format-Table -Property Interface*, IPAddress, PrefixLength
...
Get-NetIPAddress -InterfaceIndex $Index -AddressFamily $IPType | Format-Table IPAddress, InterfaceIndex, PrefixLength
...
Get-NetIPAddress -InterfaceIndex $Index -AddressFamily $IPType | Format-Table

# 6. Testing that SRV2 can see the domain controller
Test-NetConnection -ComputerName SVR.Reskit.Org | Format-Table
...
Set-Item -Path $TPPATH -Value 'svr' -Force
...
Invoke-Command -ComputerName SVR -ScriptBlock $DNSSSB -Credential $Cred
...
Invoke-Command -ComputerName DC1 -ScriptBlock { Get-ADComputer -Identity SRV2 }
