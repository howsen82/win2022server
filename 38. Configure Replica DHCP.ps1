...
Get-DhcpServerv4Scope -ComputerName SVR

# 4. Getting V4 scope statistics from DC1
Get-DhcpServerv4ScopeStatistics -ComputerName SVR

# 5. Discovering a free IP address
Get-DhcpServerv4FreeIPAddress -ComputerName SVR -ScopeId 10.10.10.42

# 6. Getting SRV2 NIC Configuration
$NIC = Get-NetIPConfiguration -InterfaceAlias 'Ethernet*'

# 7. Getting IP interface
$NIC | Get-NetIPInterface | Where-Object AddressFamily -eq 'IPv4'

# 8. Enabling DHCP on the NIC
$NIC | Get-NetIPInterface | Where-Object AddressFamily -eq 'IPv4' | Set-NetIPInterface -Dhcp Enabled

# 9. Checking IP address assigned
Get-NetIPAddress -InterfaceAlias "Ethernet*" | Where-Object AddressFamily -eq 'IPv4'
