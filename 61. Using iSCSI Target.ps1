...
$Target = Get-IscsiTarget | Where-Object NodeAddress -Match 'ITTarget'
$Target
...
$RemoteDisk =  Get-Disk | Where-Object BusType -eq 'iscsi'
$RemoteDisk | Format-Table -AutoSize

# 6. Turning disk online and making disk R/W
$RemoteDisk | Set-Disk -IsOffline  $False
$RemoteDisk | Set-Disk -IsReadOnly $False
...
$RemoteDisk | New-Volume @NewVolumeHT

# 8. Using the drive as a local drive
Set-Location -Path I:
New-Item -Path I:\  -Name ITData -ItemType Directory | Out-Null
'Testing 1-2-3' | Out-File -FilePath I:\ITData\Test.Txt
