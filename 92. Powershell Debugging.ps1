...
Set-PSBreakpoint -Script $FileName -Line 4 | Out-Null # breaks at line 4
    
# 5. Adding a breakpoint on the script using a specific command
Set-PSBreakpoint -Script $FileName -Command "Get-CimInstance" | Out-Null

# 6. Adding a breakpoint when the script writes to $M
Set-PSBreakpoint -Script $FileName -Variable M -Mode Write | Out-Null
