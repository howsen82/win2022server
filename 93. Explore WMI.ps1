...
$Service = tasklist.exe /svc /fi "SERVICES eq winmgmt" | Select-Object -Last 1
$Process = [int] ($Service.Substring(30,4))
Get-Process -Id $Process

# 5. Examining DLLs loaded by the WMI service process
Get-Process -Id $Process | Select-Object -ExpandProperty Modules | Where-Object ModuleName -match 'wmi' | Format-Table -Property FileName, Description, FileVersion
...
Foreach ($File in $Files) {
    $Name = $File.Name
    $Desc = ($File | Select-Object -ExpandProperty VersionInfo).FileDescription
