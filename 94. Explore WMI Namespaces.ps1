...
Function Get-WMINamespaceEnum {
    ...
    Param($NS)
    Write-Output $NS
    Get-CimInstance "__Namespace" -Namespace $NS @EAHT | ForEach-Object { Get-WMINamespaceEnum "$ns\$($_.name)" }
} # End of function
...
$ScriptBlock = {
    Function Get-WMINamespaceEnum {
        ...
        $EAHT = @{ ErrorAction = 'SilentlyContinue' }
        Get-CimInstance "__Namespace" -Namespace $NameSpace @EAHT | ForEach-Object {
