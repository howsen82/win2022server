...
$Query1 = "SELECT * FROM __InstanceCreationEvent WITHIN 2 WHERE TargetInstance ISA 'Win32_Process'"
$EventHT = @{
    Query            = $Query1
...
$NotepadEvent = Get-Event -SourceIdentifier 'NewProcessEvent' | Select-Object -Last 1
...
$Query2 = "SELECT * FROM RegistryValueChangeEvent WHERE Hive='HKEY_LOCAL_MACHINE' AND KeyPath='SOFTWARE\\Packt' AND ValueName='MOLTUAE'"
$Action2 = {
...
$RegisterHT = @{
    Query  = $Query2
    Action = $Action2Source = 'RegChange'
}
...
$Query1 = @"
    SELECT * From  __InstanceModificationEvent Within 5 WHERE TargetInstance ISA 'ds_group' AND TargetInstance.ds_name = '$Group'
"@
