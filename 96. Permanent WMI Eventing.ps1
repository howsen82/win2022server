...
Function Get-WMIPE {
    ...
    Get-CimInstance -Namespace $NS -ClassName $CLass1 | Where-Object Name -eq "EventFilter1" | Format-Table Name, Query
    '***Consumer Defined ***'
    $Class2 = 'CommandLineEventConsumer'
    Get-CimInstance -Namespace $NS -Classname  $Class2 | Where-Object { $_.Name -eq "EventConsumer1" } | Format-Table Name, Commandlinetemplate
    '***Bindings Defined ***'
    $Class3 = '__FilterToConsumerBinding'
    Get-CimInstance -Namespace root\subscription -ClassName $Class3 | Where-Object -FilterScript { $_.Filter.Name -eq "EventFilter1" } | Format-Table Filter, Consumer
}
...
Function Remove-WMIPE {
    ...
    $Class2 = 'CommandLineEventConsumer'
    $Class3 = '__FilterToConsumerBinding'
    Get-CimInstance -Namespace $NS -Classname $class1 | Where-Object Name -eq "EventFilter1" | Remove-CimInstance
    Get-CimInstance -Namespace $NS -ClassName $Class2 | Where-Object Name -eq 'EventConsumer1' | Remove-CimInstance
    Get-CimInstance -Namespace $NS -ClassName $Class3 | Where-Object -FilterScript { $_.Filter.Name -eq 'EventFilter1' } | Remove-CimInstance
}

# 3. Creating an event filter query
$Group = 'Enterprise Admins'
$Query = @"
  SELECT * From __InstanceModificationEvent Within 10 WHERE TargetInstance ISA 'ds_group' AND TargetInstance.ds_name = '$Group'
"@
