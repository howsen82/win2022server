...
Invoke-Command -Session $Session -ScriptBlock {
    Get-WsusProduct | Where-Object -FilterScript {$_.product.title -match '^Windows Server'} | Select-Object -ExpandProperty Product | Format-Table Title, UpdateSource
}

# 3. Discovering updates for for Windows 11
Invoke-Command -Session $Session -ScriptBlock {
    Get-WsusProduct -TitleIncludes 'Windows 11' | Select-Object -ExpandProperty Product | Format-Table -Property Title
}

# 4. Create and view a list of software product titles to include
$Products = (Get-WsusProduct | Where-Object -FilterScript {$_.product.title -match '^Windows Server'}).Product.Title
$Products += @('Microsoft SQL Server 2016','Windows 11')
$Products

# 5. Assigning the desired products to include in Windows Update
Invoke-Command -Session $Session -ScriptBlock {
    Get-WsusProduct | Where-Object {$PSItem.Product.Title -in $Products} | Set-WsusProduct
}

# 6. Getting WSUS classification
Invoke-Command -Session $Session -ScriptBlock {
    Get-WsusClassification | Select-Object -ExpandProperty Classification | Format-Table -Property Title, Description -Wrap
}
...
Invoke-Command -Session $Session -ScriptBlock {
    Get-WsusClassification | Where-Object {$_.Classification.Title -in $UpdateList} | Set-WsusClassification
}
